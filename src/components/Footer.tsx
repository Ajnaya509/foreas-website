 ...
 * - Logo "FOREAS/" gradient signature 3-stops
 * - Liens nav avec underline-grow (signature)
 * - Mentions légales complètes (CGU + Confidentialité + Mentions)
 * - Touch targets 44pt sur tous les liens
 * - Focus rings cyan 2px (WCAG AA)
 * - bg-foreas-obsidian + glass-border
...
export default function Footer() {
  return (
    <footer className="relative bg-foreas-obsidian border-t border-glass-border" role="contentinfo">
      <div className="max-w-7xl mx-auto px-lg lg:px-xxxl">
        {/* Main */}
        <div className="py-huge md:py-massive">
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-xxxl md:gap-massive">
            {/* Brand — logo gradient signature */}
            ...
            <nav
              ...
              className="flex flex-wrap justify-center md:justify-start gap-x-xl gap-y-md sm:gap-x-xxl md:gap-x-xxxl"
            >
              {links.map((link) => (
                <Link
                  key={link.name}
                  href={link.href}
                  className={`text-label text-text-tertiary hover:text-text-primary transition-colors duration-fast ease-standard min-h-[44px] flex items-center px-xs ${focusRing}`}
                >
            ...
            <div className="flex items-center justify-center md:justify-start gap-md" aria-label="Réseaux sociaux">
              <a
                ...
                className={`w-11 h-11 rounded-full glass-card-mid !p-0 flex items-center justify-center text-text-tertiary hover:text-text-primary hover:!border-white/[0.18] transition-all duration-fast ease-standard ${focusRing}`}
