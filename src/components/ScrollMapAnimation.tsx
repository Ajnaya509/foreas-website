...
export default function ScrollMapAnimation() {
  ...
  // ─── Map pan ──────────────────────────────────────────────
  // Mobile: smaller pan (240px) to keep zone chaude visible in narrow viewport
  // Desktop: moderate pan (300px) with 140vw SVG width
  const mapPanPx = isMobile ? -240 : -300
  ...
  // Car path: zone vide (160,300) → zone chaude (480,150)
  const carX = useTransform(carProgress, [0, 0.25, 0.55, 1], [160, 260, 380, 480])
  const carY = useTransform(carProgress, [0, 0.25, 0.55, 1], [300, 260, 200, 150])
      ...
      <div className="sticky top-0 h-screen flex flex-col items-center justify-center overflow-hidden">
        ...
        >
          ...
          >
            <svg
              className="h-full"
              style={{ width: isMobile ? '190vw' : '140vw' }}
              viewBox="0 0 700 500"
              preserveAspectRatio="xMinYMid slice"
            >
              <rect width="700" height="500" fill="#060610" />
              ...
              {/* ─── Street grid ─── */}
              <path d="M 0 100 L 700 100" stroke="#1a1a2e" strokeWidth="1.5" />
              <path d="M 0 200 L 700 200" stroke="#1e1e35" strokeWidth="2.5" />
              <path d="M 0 300 L 700 300" stroke="#1a1a2e" strokeWidth="1.5" />
              <path d="M 0 400 L 700 400" stroke="#1e1e35" strokeWidth="2.5" />
              ...
              {/* Diagonal boulevard */}
              <path d="M 40 430 Q 200 340 350 230 Q 480 150 650 70" stroke="#1a1a35" strokeWidth="3.5" fill="none" opacity="0.5" />

              {/* Seine river */}
              <path d="M -10 420 Q 130 370 260 385 Q 380 400 480 355 Q 600 310 720 340" stroke="url(#immSeineGrad)" strokeWidth="28" fill="none" />
              ...
              {/* ─── ZONE CHAUDE — repositioned to cx=480 for visibility ─── */}
              <motion.g style={{ opacity: hotZoneOpacity }}>
                <circle cx="480" cy="150" r="100" fill="url(#hotGlow)" />
                <circle cx="480" cy="150" r="60" fill="rgba(239,68,68,0.1)" stroke="rgba(239,68,68,0.5)" strokeWidth="2.5" />
                {!skipInfinite && (
                  <motion.circle
                    cx="480" cy="150" r="60"
                    fill="none"
                ...
                <text x="480" y="138" textAnchor="middle" fill="#f87171" fontSize="14" fontWeight="700" fontFamily="system-ui">ZONE CHAUDE</text>
                <text x="480" y="170" textAnchor="middle" fill="#fff" fontSize="24" fontWeight="800" fontFamily="system-ui">+35%</text>

                {/* People dots */}
                {[0, 1, 2, 3, 4, 5].map(i => {
                  const angle = (i / 6) * Math.PI * 2 - Math.PI / 2
                  const cx = 480 + Math.cos(angle) * 78
                  const cy = 150 + Math.sin(angle) * 78
              ...
              <motion.path
                d="M 160 300 Q 260 260 340 220 Q 410 185 480 150"
                fill="none"
