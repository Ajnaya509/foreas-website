...
/**
 * ⚠️ CES DEUX VALEURS ONT ÉTÉ TROUVÉES PAR L'EXPÉRIENCE, PAS LUES.
 *
 * Premier essai en production : `23514`, une contrainte refusait. Le dépôt ne
 ...
 * Quatre couples essayés en ligne, du plus vrai au plus sûr. Verdict :
 * `source_canal` n'accepte PAS `site_mobile` ; `claim_method` accepte bien
 * `code_court_site`. Le couple retenu est donc `widget` / `code_court_site`,
 * prouvé par un billet réel (`QDB9K7`) et son message.
 *
 * `widget` est vrai : le téléphone de `/mobile` EST un widget du site.
 * Si le fil Pieuvre ouvre un jour `site_mobile`, c'est la seule ligne à changer.
 */
const SOURCE_CANAL = 'widget'
const CLAIM_METHOD = 'code_court_site'
...
export async function POST(request: NextRequest) {
  ...
  for (let essai = 0; essai < 3; essai++) {
    ...
    if (!error) {
      return NextResponse.json({ ok: true, short_code }, { headers: { 'Cache-Control': 'no-store' } })
    }
