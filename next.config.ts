...
const nextConfig: NextConfig = {
  ...
  async redirects() {
    ...
    return [
      ...
      sert. Deux pages qui encaissent, ce sont deux grilles à tenir à jour, et
      un jour deux prix différents : le site l'a déjà vécu le 14/08.
      Les paramètres (`?formule=`, `?ref=`) traversent la redirection. */
      { source: '/tarifs', destination: '/tarifs3', permanent: true },
      { source: '/tarifs2', destination: '/tarifs3', permanent: true },
      ...
      //
      // Cette page avait sa propre grille (97 € / Elite 247 €) pendant que la page
      // de tarifs vendait 29,99 €. Le prix a été aligné le 14/08, mais le vrai problème n'est
      ...
      { source: '/checkout', destination: '/tarifs3', permanent: true },
    ]
  },
  async headers() {
    return [
